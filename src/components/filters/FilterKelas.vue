	...
	<q-card flat bordered>
		...
		<q-card-section class="no-padding flex flex-center" :key="keyReload">
			<div class="row" style="max-width: 1000px; width: 100%">
				<select-tahun-ajaran class="col-12 col-md-3 q-pa-sm" :start-url="startUrl" />
				<select-tingkat v-if="showTingkat" class="col-12 col-md-3 q-pa-sm" :start-url="startUrl" />
				<select-kelas v-if="showKelas" class="col-12 col-md-3 q-pa-sm" :start-url="startUrl" />
				<select-ujian-ke
					v-if="showUjianKe"
...
function displayFilter() {
	...
	if (params.th_ajaran_h) {
		text = '➡️ <em class="text-weight-light">Tahun Ajaran: </em><strong>' + params.th_ajaran_h + '</strong>';
	}

	if (params.tingkat_id) {
		const allTingkat = listsMadrasahStore().getAllTingkat();
		if (allTingkat.length) {
			const tingkat = allTingkat.find((t) => t.tingkat_id == params.tingkat_id);
			text += ' ➡️ <em class="text-weight-light">Tingkat: </em><strong>' + tingkat.tingkat + '</strong>';
		}
	}

	if (params.kelas) {
		text += ' ➡️ <em class="text-weight-light">Kelas: </em><strong>' + params.kelas + '</strong>';
	}

	if (params.ujian_ke) {
		const u = listsMadrasahStore().getUjian.find((u) => u.value == params.ujian_ke);
	...
	if (params.list_bulan_ujian) {
		const tk = listsMadrasahStore().getBulanUjianListByTahunAndTingkatId(params.th_ajaran_h, params.tingkat_id);
