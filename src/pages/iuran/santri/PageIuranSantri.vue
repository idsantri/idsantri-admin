<template lang="">
	<div>
		...
		/>
		<div class="q-mt-sm">
			<CardSantriSimple v-if="params.id" :id="params.id" @loaded="(r) => (santri = r)" />
			<q-card bordered flat v-else>
				<q-card-section class="q-pa-sm bg-green-11 text-subtitle2"> No data </q-card-section>

				<q-card-section class="q-pa-lg text-body1 text-center"> Pilih Santri </q-card-section>
			</q-card>
		</div>
		<q-card flat bordered class="q-mt-sm">
			<q-card-section class="q-pa-sm text-subtitle1 bg-green-6 text-green-11 flex items-center">
				Data Iuran
				<q-space />
				<q-btn icon="sync" outline dense @click="tabKey++" class="q-px-sm q-mr-sm" />
		...
		<q-dialog v-model="crud">
			<!-- edit -->
			<IuranForm :data="dataIuran" @success-create="onCreate" :disable-santri-id="true" />
		</q-dialog>
