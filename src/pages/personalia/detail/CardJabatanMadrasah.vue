<template lang="">
	<div class="q-card--bordered">
		<div class="q-px-md">
			<q-toolbar class="no-padding no-margin">
				<q-toolbar-title class="text-subtitle1 text-green-10 text-weight-bold">
					Jabatan Madrasah
				</q-toolbar-title>
				<q-btn
			...
			<q-item v-else v-for="item in am" :key="item.id" class="">
				<q-item-section>
					...
					<q-item-label lines="1">
						<span class="text-bold"> {{ item.jabatan }} </span>; {{ item.tingkat
	...
	<q-dialog v-model="crudShow">
		<PersonaliaMadrasahForm :data="dataObj" @success-submit="loadData" @success-delete="loadData" />
	</q-dialog>
</template>
<script setup>
...
async function loadData() {
	if (route.params.id) {
		try {
			loading.value = true;
			const data = await AparaturMadrasah.getAll({ params: { aparatur_id: route.params.id } });
